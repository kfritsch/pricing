...
class Station(object):
	...
	def get_pricing(self, d_int, rem_outlier=False):
		...
		if prev_price is None:
			prev_price = list(CURSOR.fetchone())
		...
		for i in range(0,cnt):
			...
			fol_price=list(CURSOR.fetchone())
		...
		for i in range(0,len(self.neighbors)):
			...
			for gas in range(len(GAS)):
				...
				while((any(gas_rule_days) or any(gas_rule_hours)) and dist_idx>0):
					...
					# 	dr = [gas_reactions[gas][j] for j in range(len(gas_reactions[gas])) if react_difs[j]==1]
					# 	di = [gas_ignores[gas][j] for j in range(len(gas_ignores[gas])) if ignore_difs[j]==1]

					# 	f.write(html_heading(1, "Special " + str(dist_val)))
					# 	# write a header for the reaction type
					# 	f.write(html_heading(2, "After own reaction"))
					# 	# write the outlier as html table
					# 	for react in dr:
					# 		f.write(self._get_leader_env_as_html_table(neigh_id, (react[0], react[1],0,0)))
					# 		f.write('<br>')
					# 	# write a header for the reaction type
					# 	f.write(html_heading(2, "After ignored pricing"))
					# 	# write the outlier as html table
					# 	for ignore in di:
					# 		f.write(self._get_leader_env_as_html_table(neigh_id, (ignore[0]-1,ignore[1],0,0)))
					# 		f.write('<br>')
					...
					if(confidence<0.5):
						...
						# write a intro
						f.write(html_intro(GAS[gas] + " rule outlier"))
						# write a header for the distance
						f.write(html_heading(1, "price distance outlier with distance: " + str(dist_val)))
						# write a header for the reaction type
						f.write(html_heading(2, "After own reaction"))
						# write the outlier as html table
					...
					# otherwise create a rule out of the distance
						...
						rule = Rule(owner=self.id,
							...
							total=np.sum(drop_hist))

						compare_dist = [min(oc,nc) for (oc,nc) in zip(drop_hist, neigh_drop_hist)]

						# # PRINT: rule hours to fill
						# print(print_bcolors(['BOLD', 'FAIL'],str([i for i in range(len(gas_rule_hours)) if gas_rule_hours[i]])))
						exceptions+=rule.get_time_windows(dist_reacts,dist_ignores,gas_rule_days,gas_rule_hours,compare_dist)
						self.rules_pn[neigh_id][gas].append(rule)
						# # PRINT: rule hours to fill
...
class Rule(object):
	...
	def __init__(self, owner, competitor, gas_type, date_int, analysis_path, max_p_dist, confidence, reactions, ignores, exceptions, total):
		...
		"""
		@ivar   hour_rule:  The mean reaction time of the pricings in accordance with this rule
		@dtype  hour_rule: int
		"""
		self.mean_rtime = 0
		"""
		@ivar   hour_rule: The standard deviation of the pricings in accordance with this rule
		@dtype  hour_rule: int
		"""
		self.dev_rtime = 0
	...
	def write_stats_analysis(self):
		...
		own_str = str(STATION_DICT[self.owner])
		neigh_str = str(STATION_DICT[self.competitor])
	...
	def get_time_windows(self, reactions, ignores, day_slots, hour_slots, own_dist):
		"""
		...
		@param own_dist: for every hour the minor value of the two competitors distibutions
		@dtype own_dist: list(Int)

		@return outlier: sum up those reactions in still available slots they are new exceptions
		@dtype outlier: string
		"""
		required_hour_slots = 3
		...
		if(any(hour_slots)):
			...
			rule_hours = [pricing_cnt>=(own_cnt/10) for (pricing_cnt,own_cnt) in zip(comb_hour_hist,own_dist)]
	...
	def _get_reactions_in_rule(self, reactions):
		...
		"""
		station = STATION_DICT[self.owner]
		rule_reactions = []
...
if __name__ == "__main__":
	...
	try:
		...
		for i in range(0,len(test_stations_plz)):
			...
			CURSOR.execute("SELECT id FROM gas_station WHERE post_code=%s AND brand=%s"  ,(test_stations_plz[i],test_station_brand[i]))
